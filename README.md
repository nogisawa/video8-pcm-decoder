...
復調はCRCを判定器にした二段階探索です。粗い位相探索でCRC-validなブロック群から
フィールド位置をロックし、ロック位相に加えて2位相だけを収集します。各ブロックは予測位置を
最初に1回だけ復号し、CRCが一致しないブロックに限って最大49段階の局所位相探索へ進みます。
このため、正常なブロックに総当たり探索は行いません。
